...
TEST(uint_trie_key_test, predefined_key)
{
    ...
    EXPECT_FALSE(key1.is_empty()) << "is_empty returned true on predefined key";
...
TEST(uint_trie_key_test, pop_invalid)
{
    ...
    EXPECT_EQ(8U, uint8_key::radix_bit_size()) << "radix_bit_size calculation is wrong";
    uint8_key key1;
    EXPECT_EQ(uint8_key::pop_result::key_empty, std::get<0>(key1.pop())) << "pop succeeded on empty key";
    uint8_key key2(255U);
    EXPECT_EQ(uint8_key::pop_result::success, std::get<0>(key2.pop())) << "pop failed";
    EXPECT_EQ(uint8_key::pop_result::key_empty, std::get<0>(key2.pop())) << "pop succeeded on empty key";
}
...
TEST(uint_trie_key_test, pop_basic)
{
    ...
    EXPECT_TRUE(key1.is_empty()) << "fully popped key is not empty";
    ...
    EXPECT_TRUE(key2.is_empty()) << "fully popped key is not empty";
}
